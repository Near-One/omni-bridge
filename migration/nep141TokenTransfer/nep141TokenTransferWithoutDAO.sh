...
for token in "${tokens[@]}"; do
    ...
    near contract call-function as-transaction $TOKEN_LOCKER transfer_tokens json-args "{\"token_id\": \"$token\", \"receiver_id\": \"$OMNI_BRIDGE_NEAR\", \"amount\": $balance}" prepaid-gas '100.0 Tgas' attached-deposit '1 yoctoNEAR' sign-as $TOKEN_LOCKER_ADMIN network-config $NEAR_NETWORK sign-with-keychain send
