...
async function getLatestEvents() {
	try {
		...
		console.log(
			chalk.blue(`Scanning blocks from ${fromBlock} to ${latestBlock}...`),
		);
		...
		for (const event of events) {
			...
			try {
				...
				if (logIndexInArray === -1) {
					console.warn(
						chalk.yellow(
							`Warning: Log not found in receipt for transaction ${event.transactionHash}`,
						),
					);
					continue;
				}
				...
				if (!isFinalized) {
					...
					console.log(
						chalk.yellow(
							`Transaction ${event.transactionHash} is NOT finalized`,
						),
					);
				} else {
					console.log(
						chalk.green(`Transaction ${event.transactionHash} is finalized`),
					);
				}
			} catch (error) {
				console.error(
					chalk.red(`Error processing transaction ${event.transactionHash}:`),
					error,
				);
				process.exit(1);
			}
		...
		if (notFinalizedCount > 0) {
			console.log(
				chalk.yellow(
					`${notFinalizedCount} transactions are NOT finalized. Please wait for finalization!`,
				),
			);
			...
			console.log(
				chalk.blue("No transactions to process in the specified range."),
			);
		}
	} catch (error) {
...
(async () => {
	try {
		console.log(
			chalk.blue("Starting NEAR-ETH bridge transaction verification..."),
		);
		console.log(
			chalk.blue(`Network: ETH ${ENV.NETWORK_ETH} → NEAR ${ENV.NETWORK_NEAR}`),
		);
