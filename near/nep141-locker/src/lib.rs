...
mod tests {
    ...
    fn run_update_transfer_fee(
        ...
        let attached_deposit = attached_deposit.unwrap_or_else(|| match &new_fee {
            UpdateFee::Fee(new_fee) => NearToken::from_yoctonear(
                new_fee.native_fee.0.saturating_sub(init_fee.native_fee.0),
            ),
    ...
    fn get_prover_result(recipient: Option<OmniAddress>) -> ProverResult {
        use std::str::FromStr;
        let recipient =
            recipient.unwrap_or(OmniAddress::Near(DEFAULT_NEAR_USER_ACCOUNT.to_string()));
        ProverResult::InitTransfer(InitTransferMessage {
            emitter_address: OmniAddress::Eth(
    ...
    fn test_fin_transfer_callback_near_success() {
        ...
        let result = contract.fin_transfer_callback(
            &storage_args,
            predecessor.clone(),
