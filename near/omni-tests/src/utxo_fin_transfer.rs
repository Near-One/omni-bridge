...
mod tests {
    ...
    use crate::helpers::tests::{account_n, base_eoa_address, build_artifacts, NEP141_DEPOSIT};
    ...
    mod transfer_to_near {
        ...
        async fn succeeds_basic(build_artifacts: &BuildArtifacts) -> anyhow::Result<()> {
            ...
            env.token_contract
                .call("ft_transfer")
                .args_json(json!({
                    "receiver_id": env.relayer_account.id(),
                    "amount": U128(amount),
                    "memo": None::<String>,
                }))
            ...
            let _ = do_utxo_fin_transfer(
                &env,
                amount,
                OmniAddress::Near(recipient.clone()),
                String::default(),
                None,
            )
            .await?;
        ...
        async fn fails_when_sender_is_not_connector(
            ...
            let utxo_fin_transfer_msg = UtxoFinTransferMsg {
                utxo_id: "test_utxo_123".to_string(),
            ...
            let result = env
                ...
                .args_json(json!({
                    ...
                    "amount": U128(amount),
                    "memo": None::<String>,
                    "msg": serde_json::to_string(&BridgeOnTransferMsg::UtxoFinTransfer(utxo_fin_transfer_msg))?,
                }))
                .deposit(NearToken::from_yoctonear(1))
                .max_gas()
                .transact()
    ...
    mod fast_transfer_finalization {
        ...
        #[rstest]
        #[tokio::test]
        async fn succeeds_pays_relayer_for_near(
            build_artifacts: &BuildArtifacts,
        ) -> anyhow::Result<()> {
            let env = TestEnv::new(build_artifacts).await?;
            let recipient = account_n(1);
            let amount = 100_000_000;
            let relayer_fee = 1_000_000;

            // Register recipient and relayer
            for account in [&recipient, env.relayer_account.id()] {
                env.token_contract
                    .call("storage_deposit")
                    .args_json(json!({
                        "account_id": account,
            ...
            // Step 1: Relayer performs fast transfer
            let fast_transfer_msg = FastFinTransferMsg {
                transfer_id: UnifiedTransferId {
                    origin_chain: ChainKind::Btc,
                    id: ChainTransferId::Utxo("test_utxo_789".to_string()),
                },
                recipient: OmniAddress::Near(recipient.clone()),
                fee: Fee {
                    fee: U128(relayer_fee),
                    native_fee: U128(0),
                },
                amount: U128(amount),
                msg: String::default(),
            ...
            // Mint tokens to relayer for fast transfer
            env.token_contract
                .call("ft_transfer")
                .args_json(json!({
                    "receiver_id": env.relayer_account.id(),
            ...
            // Deposit storage for fast transfer
            ...
            env.relayer_account
                ...
                .deposit(required_storage.saturating_add(NEP141_DEPOSIT))
                .max_gas()
                .transact()
                .await?
                .into_result()?;
            ...
            // Step 2: Connector finalizes the transfer
            let utxo_msg = UtxoFinTransferMsg {
                utxo_id: "test_utxo_789".to_string(),
                recipient: OmniAddress::Near(recipient.clone()),
            ...
            // Verify relayer received full amount (they already sent amount to recipient)
        ...
        #[rstest]
        #[tokio::test]
        async fn succeeds_pays_relayer_for_other_chain(
            build_artifacts: &BuildArtifacts,
        ) -> anyhow::Result<()> {
            let env = TestEnv::new(build_artifacts).await?;
            let amount = 100_000_000;
            ...
            // Step 1: Relayer performs fast transfer to other chain
            let fast_transfer_msg = FastFinTransferMsg {
                transfer_id: UnifiedTransferId {
                    origin_chain: ChainKind::Btc,
                    id: ChainTransferId::Utxo("test_utxo_999".to_string()),
                },
            ...
            let required_storage: NearToken = env
                ...
                .await?
                .json()?;
            let required_init_storage: NearToken = env
                .bridge_contract
            ...
            env.relayer_account
                ...
                .deposit(required_storage.saturating_add(required_init_storage))
                .max_gas()
                .transact()
                .await?
                .into_result()?;
            ...
            // Step 2: Connector finalizes the transfer
            let utxo_msg = UtxoFinTransferMsg {
                utxo_id: "test_utxo_999".to_string(),
                recipient: base_eoa_address(),
                relayer_fee: U128(relayer_fee),
                msg: String::default(),
            };
