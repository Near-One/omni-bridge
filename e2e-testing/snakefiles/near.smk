...
rule near_generate_omni_bridge_init_args:
    ...
    input:
        omni_prover_file = omni_prover_file,
    ...
    params:
        mkdir = get_mkdir_cmd(const.common_generated_dir),
        prover_address = lambda wc, input: get_json_field(input.omni_prover_file, "contract_id"),
    ...
    shell: """
    {params.mkdir} && \
    echo '{{\"prover_account\": \"{params.prover_address}\", \"btc_account_id\": \"{params.btc_account_id}\", \"btc_connector\":\"{params.btc_connector_account_id}\"}}' > {output}
    """
