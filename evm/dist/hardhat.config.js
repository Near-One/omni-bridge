...
require("./src/eNear/scripts");
dotenv.config();
const ALCHEMY_API_KEY = process.env.ALCHEMY_API_KEY;
const INFURA_API_KEY = process.env.INFURA_API_KEY;
const ETH_PRIVATE_KEY = process.env.ETH_PRIVATE_KEY || "11".repeat(32);
const ETHERSCAN_API_KEY = process.env.ETHERSCAN_API_KEY;
(0, config_1.task)("set-metadata-ft", "Set metadata for NEP-141 tokens on the Ethereum side")
...
(0, config_1.task)("deploy-bridge-token-factory", "Deploys the BridgeTokenFactory contract")
    ...
    const BridgeTokenFactoryContract = await ethers.getContractFactory("BridgeTokenFactory");
    const BridgeTokenFactory = await upgrades.deployProxy(BridgeTokenFactoryContract, [taskArgs.bridgeTokenImpl, taskArgs.nearBridgeDerivedAddress, taskArgs.omniBridgeChainId], {
    ...
    console.log("Implementation address:", await upgrades.erc1967.getImplementationAddress(await BridgeTokenFactory.getAddress()));
});
(0, config_1.task)("deploy-token-impl", "Deploys the BridgeToken implementation").setAction(async (_, hre) => {
    const { ethers } = hre;
...
(0, config_1.task)("upgrade-bridge-token", "Upgrades a BridgeToken to a new implementation")
...
(0, config_1.task)("upgrade-factory", "Upgrades the BridgeTokenFactory contract")
    .addParam("factory", "The address of the BridgeTokenFactory contract")
    .setAction(async (taskArgs, hre) => {
    const { ethers, upgrades } = hre;
    const BridgeTokenFactoryContract = await ethers.getContractFactory("BridgeTokenFactory");
...
(0, config_1.task)("etherscan-verify", "Verify contract on etherscan")
