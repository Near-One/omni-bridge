...
(0, config_1.task)("deploy-e-near-proxy", "Deploys the ENearProxy contract")
    ...
    const eNearProxy = await upgrades.deployProxy(eNearProxyContract, [taskArgs.enear, nearConnector, 0], {
        initializer: "initialize",
        timeout: 0,
    });
    await eNearProxy.waitForDeployment();
    console.log(`eNearProxy deployed at ${await eNearProxy.getAddress()}`);
    console.log("Implementation address:", await upgrades.erc1967.getImplementationAddress(await eNearProxy.getAddress()));
});
(0, config_1.task)("e-near-set-admin", "Set the proxy as admin for eNear")
...
(0, config_1.task)("deploy-fake-prover", "Deploy fake prover").setAction(async (_taskArgs, hre) => {
    const { ethers } = hre;
